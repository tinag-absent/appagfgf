...
(function() {
  ...
  function showIncidentPopup(incident, marker) {
    ...
    document.getElementById('popupTitle').innerHTML = `
      <span style="color: ${severityColors[incident.severity]};">[${severityLabels[incident.severity]}]</span> ${escapeHtml(incident.name)}
    `;
    
    document.getElementById('popupContent').innerHTML = `
      <div class="popup-stat">
        <span>発生場所:</span>
        <span style="color: white;">${escapeHtml(incident.location)}</span>
      </div>
      <div class="popup-stat">
        <span>状態:</span>
  ...
  function createIncidentList() {
    ...
    list.innerHTML = sorted.map(incident => {
      ...
      return `
        <div class="incident-card ${incident.severity}" data-incident-id="${escapeHtml(incident.id)}">
          <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem;">
            <div>
              <div style="font-weight: 600; color: white; margin-bottom: 0.25rem;">${escapeHtml(incident.name)}</div>
              <div class="font-mono" style="font-size: 0.7rem; color: var(--muted-foreground);">${escapeHtml(incident.id)}</div>
            </div>
  ...
  if (window.MapData.isLoaded) {
    init();
  } else {
    window.addEventListener('mapDataLoaded', init, { once: true });
  }
