...
(function() {
  ...
  function init() {
    ...
    // Print welcome message — これらは welcome-line クラスで保護される
    printWelcomeLine(asciiLogo, 'success');
    printWelcomeLine('═'.repeat(80), 'system');
    printWelcomeLine(`SYSTEM ACCESS GRANTED`, 'success');
    printWelcomeLine(`User: ${currentUser.name} [${currentUser.id}]`, 'info');
    printWelcomeLine(`Clearance Level: ${currentUser.clearance || 1}`, 'info');
    printWelcomeLine(`Current Level: ${currentUser.level}`, 'info');
    printWelcomeLine(`Session Start: ${new Date().toLocaleString('ja-JP')}`, 'system');
    printWelcomeLine('═'.repeat(80), 'system');
    printWelcomeLine('');
    printWelcomeLine('Type "help" to see available commands.', 'system');
    printWelcomeLine('');
  ...
  function cmdClear(args) {
    // welcome-line クラスの付いた起動ブロックは残し、それ以外を削除する
    const nonWelcome = output.querySelectorAll('.console-line:not(.welcome-line)');
    nonWelcome.forEach(el => el.remove());
  }
  ...
  // ウェルカムブロック専用 — clear コマンドで消えない
  function printWelcomeLine(text, type = 'system') {
    const line = document.createElement('div');
    line.className = `console-line ${type} welcome-line`;
    line.textContent = text;
    output.appendChild(line);
    output.scrollTop = output.scrollHeight;
  }
