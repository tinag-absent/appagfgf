...
const ProgressSystem = (function() {
  ...
  // Config loaded from progress-config.json
  let _config = null;
  let _configLoaded = false;

  // Fallback defaults (used until JSON is loaded)
  const _defaults = {
    levelThresholds: { 0: 0, 1: 100, 2: 300, 3: 600, 4: 1200, 5: 2500 },
    levelUnlocks: {
      0: ['index.html', 'login.html', 'dashboard.html'],
      1: ['divisions.html', 'chat.html', 'map.html', 'details/location-detail.html', 'history.html', 'codex.html', 'reports.html', 'agency-history.html', 'skill-tree.html', 'forms.html'],
      2: ['division-convergence.html', 'division-support.html', 'division-engineering.html', 'division-foreign.html', 'division-port.html', 'details/entity-detail.html', 'details/module-detail.html', 'entities.html', 'modules.html', 'statistics.html'],
      3: ['phenomenon.html'],
      4: ['missions.html', 'search.html', 'details/mission-detail.html'],
      5: ['classified.html', 'details/personnel-detail.html']
    },
    xpRewards: { first_login: 50, profile_view: 10, chat_message: 5, division_view: 20, phenomenon_view: 30, mission_complete: 100, daily_login: 25, location_view: 15, entity_view: 15, module_view: 15, search_use: 8, bookmark_add: 5 },
    dailyLoginRewards: { 1: 25, 2: 30, 3: 35, 4: 40, 5: 45, 6: 50, 7: 100 }
  };

  // Accessors — use loaded config if available, otherwise fallback
  function cfg() { return _config || _defaults; }
  const LEVEL_THRESHOLDS    = new Proxy({}, { get: (_, k) => cfg().levelThresholds[k] });
  const LEVEL_UNLOCKS       = new Proxy({}, { get: (_, k) => cfg().levelUnlocks[k] });
  const XP_REWARDS          = new Proxy({}, { get: (_, k) => cfg().xpRewards[k] });
  const DAILY_LOGIN_REWARDS = new Proxy({}, { get: (_, k) => cfg().dailyLoginRewards[k] });

  // Load config - inline-first (file:// & HTTP 両対応)
  (function loadConfig() {
    if (window.__DATA_PROGRESS_CONFIG) {
      _config = window.__DATA_PROGRESS_CONFIG;
      _configLoaded = true;
      window.dispatchEvent(new CustomEvent('progressConfigLoaded'));
      return;
    }
    const basePath = './data/';
    fetch(basePath + 'progress-config.json')
      .then(r => { if (!r.ok) throw new Error('Failed to load progress-config.json: ' + r.status); return r.json(); })
      .then(data => {
        _config = data;
        _configLoaded = true;
        window.dispatchEvent(new CustomEvent('progressConfigLoaded'));
      })
      .catch(err => console.warn('progress-config.json load failed, using defaults:', err));
  })();
  ...
  function getUsers() {
    const users = safeStorage.get(USERS_KEY);
  ...
  function validateLoginData(loginData, userId) {
    ...
    if (loginData.checksum !== expectedChecksum) {
      console.error('[海蝕機関 INTEGRITY] Login data integrity check failed');
      if (typeof window._showErrorToast === 'function') {
        window._showErrorToast('DATA INTEGRITY ERROR', 'Login data integrity check failed', 'progress.js', null);
      }
      return false;
    }
    ...
    if (loginData.lastLogin > now + 300000) {
      console.error('[海蝕機関 INTEGRITY] Login timestamp is in the future');
      if (typeof window._showErrorToast === 'function') {
        window._showErrorToast('DATA INTEGRITY ERROR', 'Login timestamp is in the future', 'progress.js', null);
      }
      return false;
    }
    ...
    if (loginData.lastLogin < oneYearAgo) {
      console.error('[海蝕機関 INTEGRITY] Login timestamp is too old');
      if (typeof window._showErrorToast === 'function') {
        window._showErrorToast('DATA INTEGRITY ERROR', 'Login timestamp is too old', 'progress.js', null);
      }
      return false;
    }
  ...
  return {
    ...
    checkDailyLogin,
    getDailyLoginStatus,
    get LEVEL_UNLOCKS() { return cfg().levelUnlocks; },
    get XP_REWARDS() { return cfg().xpRewards; },
    get DAILY_LOGIN_REWARDS() { return cfg().dailyLoginRewards; },
    get isConfigLoaded() { return _configLoaded; }
  };
})();
